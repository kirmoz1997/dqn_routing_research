"""Random baseline for multi-agent set routing.

Run:
    python -m multiagent_dqn_routing.experiments.run_random_set
    python -m multiagent_dqn_routing.experiments.run_random_set --split_path data/splits/test.jsonl
"""
...
def make_random_set_router(
    ...
    def router(_text: str) -> List[int]:
        length = int(rng.integers(min_len, max_len + 1))
        agents = rng.choice(N_AGENTS, size=length, replace=False)
        return [int(a) for a in agents]
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Random baseline for multi-agent set routing",
    )
    parser.add_argument(
        "--split_path",
    ...
    print("Random set-router metrics:")
